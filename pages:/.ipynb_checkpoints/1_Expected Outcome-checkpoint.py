...
def monthly_mortgage_payment(principal: float, annual_rate: float, years: int) -> float:
    """Fixed-rate mortgage monthly payment (PMT)."""
    r = annual_rate / 12.0
    n = years * 12
    if annual_rate == 0:
        return principal / n
...
def fv_lump_sum(pv: float, annual_rate: float, years: int) -> float:
    """Future value of a lump sum with annual compounding."""
    return pv * ((1 + annual_rate) ** years)


def fv_monthly_annuity(pmt: float, annual_rate: float, years: int) -> float:
    """Future value of a level monthly contribution with monthly compounding."""
    r = annual_rate / 12.0
    n = years * 12
    if annual_rate == 0:
        return pmt * n
...
def buy_vs_rent_wealth(
    ...
    term_years: int = 30,
    rent_yield: float = 0.045,   # gross rent as % of house price per year
    invest_return: float = 0.06,
    home_appreciation: float = 0.02,
):
    """
    Returns (buying_wealth, renting_wealth, diff_buy_minus_rent)
    Matching the uploaded docx logic:
      - Buying wealth = future home value after 30 years (full equity when loan is paid off)
      - Renting wealth = FV(down payment) + FV(monthly (mortgage - rent))
    """
...
# Compute base case result from current inputs
...
col1.metric("Buying Wealth (RM)", f"{buy_wealth:,.0f}")
col2.metric("Renting Wealth (RM)", f"{rent_wealth:,.0f}")
col3.metric("Buy − Rent (RM)", f"{diff:,.0f}", help="Positive means buying leads; negative means renting + investing leads.")
...
st.info(
    "These summaries align with the uploaded report's base-case logic for Kuala Lumpur and are meant as quick decision cues.")

st.divider()

# ---------------------------------------------
# Sensitivity Tables (interactive)
# ---------------------------------------------
...
# Downloads
st.download_button(
    "Download Table A (CSV)", table_A.to_csv().encode("utf-8"),
    file_name="Sensitivity_Table_A_Mortgage_vs_InvestmentReturn.csv",
    mime="text/csv",
)
st.download_button(
    "Download Table B (CSV)", table_B.to_csv().encode("utf-8"),
    file_name="Sensitivity_Table_B_Appreciation_vs_InvestmentReturn.csv",
    mime="text/csv",
)
st.download_button(
    "Download Table C (CSV)", table_C.to_csv().encode("utf-8"),
    file_name="Sensitivity_Table_C_RentYield_vs_Mortgage.csv",
    mime="text/csv",
)
...
st.caption("Note: The app logic mirrors the assumptions in the uploaded document for direct comparability.")
